...
function AdminBooksContent() {
  ...
  async function verify(book: BookSummary) {
    ...
    setMessage(result.success ? "책이 VERIFIED 처리되었습니다." : result.error?.message ?? "책을 검증 처리하지 못했습니다.");
    if (result.success) {
      reset();
      await load();
    }
    ...
    <main>
      <Section>
        <div className="grid gap-8">
          <PageHeader eyebrow="Admin" title="책 검증" description="회원이 직접 등록한 책 정보를 확인하고 필요한 경우 수정한 뒤 VERIFIED 처리합니다." />
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
            <div className="grid gap-3">
              {books.map((book) => (
                <div className="flex flex-wrap items-start justify-between gap-4 border border-[var(--color-line)] bg-[var(--color-warm-white)] p-4" key={book.id}>
              ...
              {books.length === 0 && <EmptyState title="대상 책 없음" description="현재 조건에 해당하는 책이 없습니다." />}
            </div>
            <Card>
