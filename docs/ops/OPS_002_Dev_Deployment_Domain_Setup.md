...
상태: DRAFT  
최종 수정일: 2026-07-04  
범위: 비용 최소화 배포 전략, DuckDNS 무료 도메인, 로컬 데스크톱 서버 운영, 필요 시 AWS 프리티어 보조 사용

---

## 1. 최종 추천 방향

Growth Archive는 소수 인원이 사용하는 프라이빗 서비스다. 현재 단계의 최우선 기준은 “비용이 거의 들지 않는 방향”이다. 따라서 Vercel/Render 유료 전환이나 AWS 상시 운영을 기본값으로 두지 않고, 가능한 한 로컬 데스크톱 서버와 무료 DuckDNS를 중심으로 간다.
...
SUPABASE_STORAGE_PUBLIC_BASE_URL=https://{project-ref}.supabase.co/storage/v1/object/public/images
