...
| 문서 상태 | Review 반영 완료. PRD-003 작성 전 기준 문서로 사용한다. |
...
| D-002-003 | 초대코드 | 활성 초대코드는 1개만 유지한다. Admin이 코드를 변경하면 기존 코드는 즉시 무효화된다. |
| D-002-004 | 온보딩 부담 | 기존 회원도 쉽게 가입할 수 있도록 필수 입력 항목을 최소화한다. |
| D-002-005 | 가입 이유 | 선택 입력으로 변경한다. 필수가 아니다. |
| D-002-006 | 직업 | 직접 입력하되 선택 입력으로 둔다. Guest 공개 프로필에는 노출하지 않는다. |
| D-002-007 | 나이 | MVP에서는 수집하지 않는다. |
| D-002-008 | 프로필 공개 | 비공개 옵션은 없다. 실명 공개 또는 닉네임 공개 중 선택한다. |
| D-002-009 | Guest 공개 프로필 | 성장하는 사람들 목록/프로필은 공개하되, 직업/가입 이유/현재 고민/3년 뒤 목표/실행계획/회고는 Member 전용으로 둔다. |
...
| 초대코드 | `RICH2026` 같은 공용 코드 1개를 Admin이 관리한다. |
| 초대코드 변경 | 새 코드가 활성화되면 기존 활성 코드는 즉시 무효화된다. |
| 운영진 권한 | 운영진은 동일 권한을 가진다. Owner/Admin 구분 없음. |
| 사용자 유형 | Guest, Kakao User, Member, Admin으로 구분한다. |
| 사용자 단계 | `KAKAO_AUTHENTICATED`, `INVITE_VERIFIED`, `ACTIVE_MEMBER`, `DEACTIVATED`는 저장 enum이 아니라 timestamp 필드 기반 계산 결과로 사용한다. |
...
| Admin 회원 관리 | 회원 목록 조회, 비활성 처리, 프로필 확인 가능. |
| Admin 권한 부여/회수 | MVP에서는 운영 스크립트 또는 DB seed로 처리한다. UI 제공은 MVP 제외. |
| 탈퇴 | MVP에서는 셀프 탈퇴 기능을 제외하고, 운영진 요청 기반 비활성화를 기본값으로 둔다. |
...
아래 단계는 DB에 `account_status` enum으로 저장하지 않는다. `members`의 timestamp 필드로 계산한다.

| 단계 코드 | 단계명 | 계산 기준 | 접근 가능 범위 | MVP 범위 |
|---|---|---|---|---|
| `KAKAO_AUTHENTICATED` | 카카오 로그인 완료 | OAuth 계정은 있으나 `invite_verified_at IS NULL` | 공개 페이지 + 초대코드 입력 | 포함 |
| `INVITE_VERIFIED` | 초대코드 인증 완료 | `invite_verified_at IS NOT NULL`이고 온보딩 미완료 | 온보딩 프로필 입력 | 포함 |
| `ACTIVE_MEMBER` | 활성 멤버 | `onboarding_completed_at IS NOT NULL AND deactivated_at IS NULL` | Member 기능 전체 | 포함 |
| `DEACTIVATED` | 비활성 | `deactivated_at IS NOT NULL` | 로그인 차단 또는 비활성 안내 | 포함 |
...
| `ADMIN` | 운영진 권한 | seed data 또는 운영 스크립트로 부여 | 포함 |
...
stateDiagram-v2
    ...
    ACTIVE_MEMBER --> DEACTIVATED: 운영진 비활성 처리
    DEACTIVATED --> ACTIVE_MEMBER: 운영진 재활성화 처리
```
...
6. `DEACTIVATED` 사용자는 Member/Admin 전용 기능을 사용할 수 없다.
...
3. 시스템은 OAuth 계정과 최소 member row를 생성하고, 초대코드 미인증 단계(`KAKAO_AUTHENTICATED`)로 계산되도록 둔다.
4. 사용자는 초대코드 입력 화면으로 이동한다.

#### 초대코드 인증 완료, 온보딩 미완료 사용자

1. 사용자가 다시 로그인한다.
2. 시스템은 timestamp 필드로 사용자 단계가 `INVITE_VERIFIED`임을 확인한다.
3. 사용자는 프로필 온보딩 화면으로 이동한다.
...
| 비활성 계정 | “비활성화된 계정입니다. 재가입 또는 복구는 운영진에게 문의해 주세요.” | Member 기능 차단 |
...
| 항목 | 정책 |
|---|---|
| 코드 형태 | `RICH2026` 같은 영문/숫자 조합 |
| 코드 개수 | MVP에서는 활성 코드 1개만 유지 |
| 코드 관리 | Admin이 변경 가능 |
| 코드 변경 효과 | 새 코드 활성화 시 기존 코드는 즉시 무효화 |
| 코드 입력 대상 | `KAKAO_AUTHENTICATED` 사용자 |
| 코드 성공 결과 | 즉시 `INVITE_VERIFIED` 상태로 변경 |
...
flowchart TD
    ...
    B --> C[대문자 normalize]
    C --> D{활성 초대코드와 일치?}
    D -->|Yes| E[상태 INVITE_VERIFIED로 변경]
    E --> F[온보딩 프로필 입력 화면 이동]
    D -->|No| G[오류 메시지 표시]
    G --> A
```
...
| 실명 | 조건부 | 직접 입력 | 실명 공개 선택 시 필수, 2~20자 | 표시 방식이 실명 공개일 때 O | O | 카카오 실명에 의존하지 않음 |
| 한 줄 소개 | O | 직접 입력 | 1~80자 | O | O | 프로필 카드 노출 |
| 프로필 사진 | 선택 | 이미지 업로드 | 이미지 파일, 최대 5MB 권장 | O | O | 없으면 카카오 이미지 사용 |
| 관심 분야 | O | 태그 선택 | 1~5개 선택 | O | O | 운영진 관리 태그만 선택 |
| 50살의 나 | O | 장문 입력 | 1~1000자 | O | O | 프로필 상단 핵심 |
| 직업 | 선택 | 직접 입력 | 50자 이하 | X | O | 깊은 개인 정보로 간주, Member 전용 |
...
sequenceDiagram
    ...
    BE->>BE: 내부 계정 조회/생성
    BE-->>FE: 상태 KAKAO_AUTHENTICATED 반환
    FE->>U: 초대코드 입력 화면 표시
    U->>FE: 초대코드 입력
    FE->>BE: 초대코드 검증 요청
    BE->>BE: 코드 검증 및 상태 변경
...
| 카카오 로그인 후 초대코드 미입력 | 초대코드 입력 화면 |
| 초대코드 인증 후 프로필 미작성 | 프로필 온보딩 화면 |
...
| 실명 공개 선택 후 실명 없음 | “실명 공개를 선택하려면 실명을 입력해 주세요.” |
| 약관 미동의 | “이용약관에 동의해 주세요.” |
| 개인정보처리방침 미동의 | “개인정보처리방침에 동의해 주세요.” |
| 이미지 용량 초과 | “이미지 용량은 최대 5MB까지 업로드할 수 있습니다.” |
...
| 닉네임 | 내부 식별용 |
| 직업 | 회원 입력값. Admin 조회 가능 |
| 관심 분야 | 선택 태그 |
| 회원 상태 | ACTIVE_MEMBER, DEACTIVATED |
| Role | MEMBER, ADMIN |
| 가입일 | 온보딩 완료일 기준 또는 계정 생성일 기준. TSD에서 확정 |
...
- 작성자 표시 방식은 기존 표시명 유지로 둔다.
- 익명화/삭제는 OPS-001에서 별도 정책으로 검토한다.
...
| Member 전용 항목 | 직업, 가입 이유, 현재 고민, 3년 뒤 목표, 실행계획, 회고 |
| URL | `/people/{memberId}` |
...
| ONB-003 | System | 시스템은 실명 공개 선택 시 실명 입력을 필수로 검증해야 한다. | P0 | 실명 없으면 저장 불가 |
...
Then 시스템은 Kakao Provider ID로 내부 계정을 생성하고
...
When 유효한 초대코드를 입력하면
Then 시스템은 사용자 상태를 INVITE_VERIFIED로 변경하고
...
```gherkin
Given Admin이 활성 초대코드를 RICH2026에서 RICH2027로 변경했고
When 신규 Kakao User가 RICH2026을 입력하면
...
Then 시스템은 사용자 상태를 ACTIVE_MEMBER로 변경하고
...
```gherkin
Given 사용자가 공개 표시 방식으로 실명 공개를 선택했고
When 실명 입력 없이 저장을 시도하면
Then 시스템은 저장을 거부하고
And “실명 공개를 선택하려면 실명을 입력해 주세요.” 메시지를 표시해야 한다.
```
...
직업, 가입 이유, 현재 고민, 3년 뒤 목표는 멤버에게만 보여집니다.
```
...
### 17.1 User / Account 개념 데이터

| 필드 | 설명 | 필수 |
|---|---|---:|
| internal_user_id | 내부 사용자 ID | O |
...
| invite_verified_at | 초대코드 인증 완료 시각. null이면 미인증 | 선택 |
| terms_agreed_at | 이용약관 동의 시각 | 선택 |
| privacy_agreed_at | 개인정보처리방침 동의 시각 | 선택 |
| onboarding_completed_at | 온보딩 완료 시각. 값이 있으면 Member 기능 사용 가능 | 선택 |
| deactivated_at | 비활성 처리 시각. 값이 있으면 비활성 회원 | 선택 |
| role | `MEMBER`, `ADMIN` | 조건부. 온보딩 완료 전에는 null 가능 |
| created_at | 계정 생성일 | O |
| last_login_at | 최근 로그인 | 선택 |

### 17.2 Member Profile 개념 데이터
...
6. 50살의 나
7. 프로필 이미지
8. 직업, 가입 이유, 현재 고민, 3년 뒤 목표는 선택 수집
9. 이메일은 카카오 동의 범위에 따라 선택 수집
...
| OQ-002-006 | 직업을 Member 전용으로만 둘 경우, 성장하는 사람들 Guest 카드의 보조 설명은 무엇으로 대체할 것인가? | 한 줄 소개 + 관심 분야 사용. | DESIGN-001/PRD-003 작성 시 |
...
- [ ] 직업, 가입 이유, 현재 고민, 3년 뒤 목표는 선택 입력으로 저장 가능하다.
- [ ] 실명/닉네임 공개 방식이 반영된다.
