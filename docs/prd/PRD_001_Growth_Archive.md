...
| 문서 상태 | IMPLEMENTATION_ALIGNED. 현재 코드와 배포 결과물을 기준으로 PRD 기준을 정렬한다. |
...
| 초대코드 | 일반 멤버용 코드와 운영진용 코드 2종을 관리한다. 로컬 기본값은 일반 `test`, 운영진 `admin` |
| 운영진 권한 | 운영진 5명 권한 동일 |
| 프로필 공개 | 비공개 없음. 실명 공개 또는 닉네임 공개 중 선택 |
| 닉네임 | 중복 허용 안 함 |
| 프로필 사진 | 회원 업로드 이미지 → 카카오 프로필 → 기본 이미지 순서 |
| 직업 | MVP에서 수집하지 않음 |
| 생년월일 | 필수 입력, 공개 프로필 비노출 |
...
| 독서기록 메인 | 추천책, 인기 도서 5권, 최근 독서기록 |
| 책 상세 | 평균 평점, 독서기록 수, 읽은 사람, 작성자별 기록 카드 |
| 월간 실행계획 | 자유 입력, 멤버 공개, 매월 1개 기준 |
| 월간 회고 | 월별 작성 슬롯 제공, 저장 시 row 생성, 작성 의무 없음, 멤버 공개, 언제든 수정 가능 |
...
| 후기 사진 | 최대 10장, 브라우저에서 우선 WebP 변환/리사이징/압축, 원본 저장 안 함 |
...
| 추천도서 | 운영진 추천책, 추천 이유, 노출 순서, 숨김/복구/삭제 관리 |
| 실행 | 월간 실행계획 자유 입력 |
...
| FR-004 | Member | Member는 가입 시 닉네임, 실명, 생년월일, 한 줄 소개, 관심 분야, 50살의 나를 필수로 입력하고, 가입 이유/현재 고민/3년 뒤 목표는 선택 입력할 수 있어야 한다. | P0 | 필수값 누락 시 저장 불가, 선택값은 비어 있어도 저장 가능 |
...
| FR-008 | Admin | Admin은 추천책을 등록하고 추천 이유와 노출 순서를 관리할 수 있어야 한다. | P0 | 홈/독서기록 라이브러리에 노출 |
...
| FR-013 | System | 시스템은 매월 독서기록모임과 실행수다모임을 자동 생성해야 한다. | P1 | Admin이 날짜/장소/상태 수정 가능 |
...
| 핵심 행동 | Admin 로그인 → 참여 현황 조회 → 미참여자 확인 → 카카오톡 공유 문구 복사 또는 CSV export → 추천책/모임 관리 |
...
2. Guest는 최근 성장 기록, 추천책, 인기 도서 5권을 본다.
...
|---|---:|---|---|
| 닉네임 | 필수 | 직접 입력 | 중복 불가 |
| 실명 | 필수 | 직접 입력 | 기본 공개 표시 방식은 실명 공개 |
| 한 줄 소개 | 필수 | 직접 입력 | 프로필 카드에 노출 |
| 생년월일 | 필수 | 날짜 입력 | 공개 프로필 비노출, 미래 날짜 불가 |
...
- ACTIVE 상태의 관리자 추천책을 `display_order ASC, id ASC` 순서로 노출한다.
- 추천 이유를 함께 보여준다.
...
평점은 필수 입력이며 평균 평점은 ACTIVE 독서기록의 평점을 대상으로 계산한다.
...
추천책은 단순한 책 진열이 아니라 운영진의 시선과 커뮤니티의 방향성을 보여주는 큐레이션 영역이다.
...
| 권장 개수 | 최대 5개 노출 슬롯. 운영진은 공개할 추천책만 ACTIVE로 유지 |
| 노출 방식 | 슬라이드/캐러셀 |
| 추천 이유 | 필수 |
| 노출 기간 | 현재 구현은 월별 필터가 아니라 `ACTIVE` 상태 전체를 노출한다. `target_month`는 운영 기록/정렬 보조 정보로 유지한다. |
| 순서 | `display_order ASC, id ASC` 기준. Admin이 직접 지정 |
...
|---|---:|---|
| 책 | 필수 | 책 검색 API로 선택 |
| 추천 월 | 필수 | YYYY-MM. 현재 공개 조회는 월별 필터를 적용하지 않음 |
| 추천 이유 | 필수 | 운영진의 추천 문장 |
| 추천자 | 선택 | 운영진 이름 또는 닉네임 |
| 노출 순서 | 필수 | 1~5. 낮을수록 먼저 노출 |
| 노출 여부 | 필수 | 공개/숨김 |
...
회원이 이번 달 무엇을 해볼지 자유롭게 남기고, 오프라인 실행수다모임에서 서로 이야기할 수 있게 한다.
...
| 완료 근거 | 독서기록 / 실행계획 |
| 운영 화면 | 월별 대상자 목록, 이름 검색, 10명 단위 페이지, 수동 참여 처리/미참여 처리, 운영 메모 저장, 카카오톡 공유 문구 복사 |
| CSV 다운로드 | API/export 용도로 유지 |
...
| 유형 | 설명 |
|---|---|
| 독서기록모임 | 매월 읽은 책과 기록을 공유하는 기본 모임 |
| 실행수다모임 | 지난달 회고와 이번 달 실행계획을 이야기하는 기본 모임 |
...
|---|---|
| 최대 개수 | 10장 |
| 저장 형식 | 브라우저 우선 WebP 변환/리사이징/압축, 서버 방어 검증 |
| 리사이징 | 자동 리사이징 |
| 원본 저장 | 저장하지 않음 |
| 대표 이미지 | 첫 번째 사진 또는 사용자가 선택한 사진 |
| EXIF | 제거 권장 |
| 부분 실패 | 성공한 사진만 저장하고 실패한 사진은 수정 화면에서 재추가 안내 |
...
| 초대코드 관리 | 현재 코드 조회/변경 |
| 추천책 관리 | 추천책 등록/수정/숨김/복구/삭제, `display_order` 순서 관리 |
...
| 참여 현황 | 월별 미참여자 목록, 수동 참여 처리, 운영 메모, 카카오톡 공유 문구, CSV export |
...
- 이미지 변환은 브라우저에서 우선 처리하고, 서버는 검증과 방어용 리사이징/변환을 한 번 더 수행한다.
...
| 이미지 | 브라우저에서 우선 WebP 변환/리사이징/압축, 서버에서 방어 검증과 재최적화 |
...
- 업로드 이미지는 브라우저에서 WebP로 변환/리사이징/압축한 뒤 서버에 전송한다.
- 서버는 최대 10MB, MIME type, 용도 검증을 수행하고 방어적으로 다시 최적화할 수 있다.
- 원본은 저장하지 않는다.
- EXIF 제거를 권장한다.
- 대표 이미지는 첫 이미지 또는 사용자 선택 이미지로 설정한다.
- 모임 후기 사진 여러 장 업로드 중 일부가 실패해도 성공한 사진만 후기와 연결하고, 실패한 사진은 저장 후 수정 화면에서 다시 추가하도록 안내한다.
- 업로드 요청은 긴 대기 상태를 막기 위해 타임아웃을 둔다. MVP 기본값은 요청당 25초다.
...
- [ ] Admin은 추천책을 등록/수정/숨김/복구하고 노출 순서를 조정할 수 있다.
- [ ] 추천 이유는 필수로 입력된다.
...
- [ ] 업로드 이미지는 브라우저에서 우선 WebP로 변환/리사이징/압축되고, 서버에서 방어 검증 후 저장된다.
...
Then 시스템은 브라우저에서 우선 WebP로 변환/리사이징/압축된 이미지를 서버 검증 후 후기와 연결해야 한다.
```
