...
| 초대코드 | `RICH2026` 형태의 공용 코드, 운영진 변경 가능 |
| 운영진 권한 | 운영진 5명 권한 동일 |
...
| 독서기록 메인 | 이달의 추천책, 인기 도서 TOP5, 최근 독서기록 |
| 책 상세 | 평균 평점, 독서기록 수, 읽은 사람, 작성자별 토글 목록 |
...
| 후기 사진 | 최대 10장, 자동 리사이징, WebP 변환, 원본 저장 안 함 |
...
| 독서기록 | 책 검색, 독서기록 등록, 책 상세, 인기 도서 TOP5, 최근 독서기록 |
...
| FR-004 | Member | Member는 가입 시 닉네임, 한 줄 소개, 관심 분야, 50살의 나를 필수로 입력하고, 직업/가입 이유/현재 고민/3년 뒤 목표는 선택 입력할 수 있어야 한다. | P0 | 필수값 누락 시 저장 불가, 선택값은 비어 있어도 저장 가능 |
...
| FR-009 | System | 시스템은 독서기록 수와 평균 평점을 기준으로 인기 도서 TOP5를 계산해야 한다. | P0 | 동률 시 최근 기록 순 정렬 |
...
2. Guest는 최근 성장 기록, 이달의 추천책, 인기 도서 TOP5를 본다.
...
flowchart TD
    A[방문자] --> B[카카오 로그인]
    B --> C{초대코드 입력 여부}
    C -->|입력 전| D[Kakao User 상태]
    C -->|유효한 코드 입력| E[Member 자동 승인]
    E --> F[가입 정보 입력]
    F --> G[프로필 생성]
    G --> H[마이페이지 진입]
```
...
| 항목 | 정책 |
|---|---|
| 코드 형태 | `RICH2026` 같은 공용 코드 |
| 코드 개수 | MVP에서는 활성 코드 1개 기준 |
| 코드 변경 | Admin 가능 |
| 코드 입력 성공 | 즉시 Member 권한 부여 |
| 코드 입력 실패 | 에러 메시지 표시 |
...
#### AC-GWT-006. 책 상세 아코디언

```gherkin
Given Guest가 책 상세 페이지에 접근했을 때
When Guest가 작성자별 독서기록 토글을 펼치면
...
Then 시스템은 이미지를 WebP로 변환하고 리사이징한 뒤 후기와 연결해야 한다.
```
...
| 이미지 저장 비용 | 후기 사진이 많아질수록 Storage 비용 증가 | WebP 변환, 원본 저장 안 함, 최대 10장 제한 |
